...
MIND_MAP_CREATE_ENDPOINT = "/api/mind-maps"
DEFAULT_TIMEOUT_SECONDS = 10

def run(params: dict[str, Any]) -> dict[str, Any]:
    """创建新的思维导图。

    调用后端 POST /api/mind-maps 接口，请求体包含 title。
    """
    title = params.get("title")
    if not title or not isinstance(title, str):
        return {
            "success": False,
            "message": "缺少必填参数或参数类型错误：title 应为字符串",
            "status_code": 400,
            "data": None,
        }
    ...
    url = urllib.parse.urljoin(f"{base_url}/", MIND_MAP_CREATE_ENDPOINT.lstrip("/"))
