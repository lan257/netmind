"""Delete all relations for a node via NetMind API."""
...
ENDPOINT = "/api/node-relations/by-node"
...
def run(params: dict[str, Any]) -> dict[str, Any]:
    ...
    if node_id is None:
        return {"success": False, "error": "缺少必需参数: nodeId"}

    request_url = _join_url(base_url, f"{ENDPOINT}/{node_id}")

    status_code, payload = _delete_json(request_url, timeout_seconds)
    ...
    return {
        ...
        "endpoint": f"{ENDPOINT}/{node_id}",
        "node_id": node_id,
        "delete_result": api_data,
...
def _delete_json(url: str, timeout_seconds: float) -> tuple[int, dict[str, Any]]:
