"""Create a node relation via NetMind API."""
...
def run(params: dict[str, Any]) -> dict[str, Any]:
    ...
    relation_type = params.get("relationType")
    map_id = params.get("mapId")
    if source_id is None:
        return {"success": False, "error": "缺少必需参数: sourceId"}
    if target_id is None:
        return {"success": False, "error": "缺少必需参数: targetId"}
    if not relation_type:
        return {"success": False, "error": "缺少必需参数: relationType"}
    if map_id is None:
    ...
    status_code, payload = _post_json(request_url, request_body, timeout_seconds)
...
def _post_json(url: str, body: dict[str, Any], timeout_seconds: float) -> tuple[int, dict[str, Any]]:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to call NetMind node-relations endpoint: {exc.reason}") from exc
...
def _decode_json_response(body: bytes) -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueError("NetMind node-relations endpoint returned non-JSON content") from exc
    if not isinstance(payload, dict):
        raise ValueError("NetMind node-relations endpoint JSON response must be an object")
    return payload
