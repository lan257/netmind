# node_relation_create - 创建关系

## 功能说明

创建节点之间的关联，调用后端 `POST /api/node-relations` 接口。
...
| `request_body` | object | 发送的请求体 |
| `relation` | object/null | 创建的关系对象（NodeRelationDto），失败时为 null |
...
| 场景 | 返回值 |
|------|--------|
| 缺少 `sourceId` | `success: false`，`error: "缺少必需参数: sourceId"` |
| 缺少 `targetId` | `success: false`，`error: "缺少必需参数: targetId"` |
| 缺少 `relationType` | `success: false`，`error: "缺少必需参数: relationType"` |
