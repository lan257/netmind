"""Get a single node by ID from NetMind API."""
...
ENDPOINT_TEMPLATE = "/api/nodes/{id}"
...
def run(params: dict[str, Any]) -> dict[str, Any]:
    ...
    endpoint = ENDPOINT_TEMPLATE.format(id=node_id)
...
def _get_json(url: str, timeout_seconds: float) -> tuple[int, dict[str, Any]]:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to call NetMind node endpoint: {exc.reason}") from exc
...
def _decode_json_response(body: bytes) -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueError("NetMind node endpoint returned non-JSON content") from exc
    if not isinstance(payload, dict):
        raise ValueError("NetMind node endpoint JSON response must be an object")
    return payload
