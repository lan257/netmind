...
MIND_MAP_DELETE_ENDPOINT = "/api/mind-maps/{id}"
DEFAULT_TIMEOUT_SECONDS = 10

def run(params: dict[str, Any]) -> dict[str, Any]:
    """删除单个思维导图（不级联删除关联节点）。

    调用后端 DELETE /api/mind-maps/{id} 接口，返回 DeleteResultDto。
    """
    ...
    endpoint = MIND_MAP_DELETE_ENDPOINT.replace("{id}", str(mind_map_id))
