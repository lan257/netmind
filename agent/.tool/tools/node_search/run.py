"""Search nodes by keyword from NetMind API."""
...
def run(params: dict[str, Any]) -> dict[str, Any]:
    ...
    if not keyword:
        return {"success": False, "error": "缺少必需参数: keyword"}

    query_params = {"keyword": keyword}
    map_id = params.get("mapId")
    if map_id is not None:
        query_params["mapId"] = str(map_id)
    limit = params.get("limit", DEFAULT_LIMIT)
    if limit is not None:
        query_params["limit"] = str(limit)

    query_string = urllib.parse.urlencode(query_params)
    request_url = _join_url(base_url, f"{ENDPOINT}?{query_string}")
...
def _get_json(url: str, timeout_seconds: float) -> tuple[int, dict[str, Any]]:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to call NetMind search endpoint: {exc.reason}") from exc
...
def _decode_json_response(body: bytes) -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueError("NetMind search endpoint returned non-JSON content") from exc
    if not isinstance(payload, dict):
        raise ValueError("NetMind search endpoint JSON response must be an object")
    return payload
